...
export namespace Vector2Tools {
	...
	export function Rotate(vector: Vector2, angle: number) {
		...
		return new Vector2(
			vector.X * angle_cos - vector.Y * angle_sin,
			vector.X * angle_sin + vector.Y * angle_cos,
		);
	}
	export function LerpWithMagnitude(
		start_vector: Vector2,
	...
	export function SubtractMagnitude(
		vector: Vector2,
		amount: number,
		clamp_to_zero: boolean = false,
	) {
