...
export namespace DetectingTools {
  ...
  export function GetPointUnderThePlayer(player: Player, max_depth: number, filter_not_anchored: boolean = true) {
    ...
    if (filter_not_anchored) {
      const parts_around = Workspace.GetPartBoundsInRadius(player_position, max_depth);
    ...
    const raycast_result = Workspace.Raycast(player_position, Vector3.yAxis.mul(-1 * max_depth), raycast_parameters);
